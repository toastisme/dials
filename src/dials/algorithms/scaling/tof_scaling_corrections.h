...
namespace dials { namespace algorithms {
  ...
  void tof_extract_shoeboxes_to_reflection_table2(
    ...
    for (std::size_t i = 0; i < reflection_table.size(); ++i) {
      ...
      for (std::size_t z = 0; z < shoebox.zsize(); ++z) {
        ...
        for (std::size_t x = 0; x < shoebox.xsize(); ++x) {
          ...
          if (panel_x > 63 || panel_x < 0) {
            continue;
          }
          for (std::size_t y = 0; y < shoebox.ysize(); ++y) {
            int panel_y = bbox[2] + y;
            if (panel_y > 63 || panel_y < 0) {
              continue;
            }

            double incident_pixel_data = i_shoebox.data(x, y, z);
            double empty_pixel_data = e_shoebox.data(x, y, z);
            double pixel_data = shoebox.data(x, y, z);
            ...
            shoebox.data(x, y, z) = (int)pixel_data;
          }
        }
      }
    }
  ...
  void tof_extract_shoeboxes_to_reflection_table(af::reflection_table &reflection_table,
    ...
    auto block1_end = std::chrono::high_resolution_clock::now();
    auto duration_block1 =
      std::chrono::duration_cast<std::chrono::microseconds>(block1_end - start);
    std::cout << "Time for init: " << duration_block1.count() << " microseconds"
              << std::endl;
    ...
    for (std::size_t img_num = 0; img_num < num_images; ++img_num) {
      ...
      DIALS_ASSERT(imgs.n_tiles() == i_imgs.n_tiles());
      DIALS_ASSERT(imgs.n_tiles() == e_imgs.n_tiles());
      ...
      auto block2_end = std::chrono::high_resolution_clock::now();
      auto duration_block2 =
        std::chrono::duration_cast<std::chrono::microseconds>(block2_end - block1_end);
      std::cout << "Time for shoebox assignment: " << duration_block2.count()
