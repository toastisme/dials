...
class TOFReflectionPredictor:
    ...
    def for_ub(self, ub):
        ...
        image_range = self.experiment.scan.get_image_range()
        frames = list(range(image_range[0], image_range[1]))
        x, y, z = reflection_table["xyzcal.px"].parts()
        xyz = flex.vec3_double(len(reflection_table))
        ...
        for i in range(len(reflection_table)):
            ...
            tof = (
                tof_helpers.tof_from_wavelength(wavelength, L0 + (L1 * 10**-3))
                * 10**6
            )  # (usec)
            frame = tof_to_frame(tof)
            xyz[i] = (x[i], y[i], float(frame))
        x, y, z = xyz.parts()
