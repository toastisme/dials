...
namespace dials { namespace algorithms {
  ...
  void integrate_reflection_table(
    ...
    int n_threads,
    boost::optional<TOFProfile1DParams> profile_1d_params = boost::none,
    boost::optional<TOFProfile3DGutmannParams> profile_3d_gutmann_params = boost::none,
    boost::optional<TOFProfile3DICParams> profile_3d_ic_params = boost::none) {
    /*
    ...
    DIALS_ASSERT(!(profile_1d_params && profile_3d_gutmann_params)
                 && !(profile_1d_params && profile_3d_ic_params)
                 && !(profile_3d_gutmann_params && profile_3d_ic_params));
    ...
    if (profile_1d_params || profile_3d_gutmann_params || profile_3d_ic_params) {
    ...
    auto worker = [&](std::size_t start, std::size_t end) {
      for (std::size_t i = start; i < end; ++i) {
        ...
        for (std::size_t z = 0; z < shoebox.zsize(); ++z) {
          ...
          for (std::size_t y = 0; y < shoebox.ysize(); ++y) {
            ...
            for (std::size_t x = 0; x < shoebox.xsize(); ++x) {
              ...
              if (profile_3d_gutmann_params || profile_3d_ic_params) {
          ...
          if (profile_1d_params) {  // Doing 1D profile fitting
        ...
        if (profile_1d_params && success) {
          bool profile_success = false;
          double I_prf;
          profile_success = fit_profile_1d(projected_intensity.const_ref(),
                                           tof_z.const_ref(),
                                           *profile_1d_params,
                                           I_prf);
          if (profile_success) {
        ...
        } else if (profile_3d_gutmann_params && success) {
          bool profile_success = false;
          ...
          succeeded_prf[i] = profile_success;
        } else if (profile_3d_ic_params && success) {
          bool profile_success = false;
          double I_prf;
          profile_success = fit_profile_3d_ic(coords_3d.const_ref(),
                                              intensity_3d,
                                              background_var_3d,
  ...
  void integrate_reflection_table(
     ...
     * a spherical absorption correction and an optional Lorentz correction
     */
        ...
        for (std::size_t z = 0; z < shoebox.zsize(); ++z) {
          ...
          for (std::size_t y = 0; y < shoebox.ysize(); ++y) {
            ...
            for (std::size_t x = 0; x < shoebox.xsize(); ++x) {
              ...
              double I = I0 / (bin_width_correction * T);
              double B = raw_B / (bin_width_correction * T);
              double var_I =
                var_I0 / (bin_width_correction * bin_width_correction * T * T);
              var_B /= (bin_width_correction * bin_width_correction * T * T);
    ...
    auto worker = [&](std::size_t start, std::size_t end) {
      for (std::size_t i = start; i < end; ++i) {
        ...
        if (profile_1d_params) {
          ...
          succeeded_prf[i] = profile_success;
        } else if (profile_3d_gutmann_params) {
          bool profile_success = false;
  ...
  boost::python::tuple calculate_line_profile_for_reflection_3d(
    ...
    scitbx::af::shared<double> tof_z_out,
    const bool& apply_lorentz_correction,
    TOFProfile3DGutmannParams& profile_3d_gutmann_params) {
    /*
     * Calculates raw_projected_intensity, projected_intensity,
     * projected_background, sum_intensity, sum_variance
     */
  ...
  boost::python::tuple calculate_line_profile_for_reflection(
    ...
    const bool& apply_lorentz_correction,
    TOFProfile1DParams& profile_1d_params) {
    /*
    ...
    if (success) {
      profile_success = fit_profile_1d(projected_intensity_out.const_ref(),
                                       tof_z_out.const_ref(),
                                       profile_1d_params,
                                       I_prf,
                                       line_profile_out,
  ...
  static bool fit_profile_3d(
    scitbx::af::const_ref<vec3<double>, af::c_grid<3>> coords,
    scitbx::af::versa<double, af::c_grid<3>> intensities,
    scitbx::af::versa<double, af::c_grid<3>> background_variances,
    TOFProfile3DGutmannParams& p,
    double& I_prf_out,
    scitbx::af::versa<double, af::c_grid<3>> profile_3d_out,
    bool update_params) {
    return fit_profile_3d_gutmann(coords,
                                  intensities,
  ...
  static bool fit_profile_3d(
    ...
    scitbx::af::versa<double, af::c_grid<3>> intensities,
    scitbx::af::versa<double, af::c_grid<3>> background_variances,
    TOFProfile3DICParams& p,
    double& I_prf_out,
    scitbx::af::versa<double, af::c_grid<3>> profile_3d_out,
    bool update_params) {
    return fit_profile_3d_ic(coords,
                             intensities,
                             background_variances,
                             p,
                             I_prf_out,
  ...
  boost::python::tuple calculate_line_profile_for_reflection_3d(
    ...
    scitbx::af::shared<double> tof_z_out,
    const bool& apply_lorentz_correction,
    TOFProfile3DICParams& profile_3d_ic_params) {
    DIALS_ASSERT(reflection.size() == 1);
    ...
    bool sum_success = true;
    int n_background = 0, n_signal = 0;
    double I_sum = 0.0, var_sum = 0.0;

    for (std::size_t z = 0; z < shoebox.zsize(); ++z) {
      if (!sum_success) break;
      for (std::size_t y = 0; y < shoebox.ysize(); ++y) {
        int panel_y = bbox[2] + y;
        if (panel_y > image_size[1] || panel_y < 0) continue;
        for (std::size_t x = 0; x < shoebox.xsize(); ++x) {
          int panel_x = bbox[0] + x;
          if (panel_x > image_size[0] || panel_x < 0) continue;
          int mask = shoebox.mask(z, y, x);
          if ((mask & Foreground) == Foreground) {
            if ((mask & Valid) == Valid && (mask & Overlapped) == 0)
              n_signal++;
    ...
    for (std::size_t z = 0; z < shoebox.zsize(); ++z) {
      double intensity_z = 0, intensity_raw_z = 0, background_z = 0;
      if (!sum_success) break;
  ...
  // Build incident and empty shoebox tables for a single-reflection table
  static void _fill_incident_empty_shoeboxes(
    dials::af::reflection_table& reflection,
    const dials_scaling::TOFIncidentSpectrumParams& incident_params,
    int n_panels,
    int num_images,
    ...
    dials::af::reflection_table& e_reflection_table) {
    boost::python::dict d;
    i_reflection_table = dxtbx::af::flex_table_suite::deepcopy(reflection, d);
    e_reflection_table = dxtbx::af::flex_table_suite::deepcopy(reflection, d);

    {
      dials::af::ref<Shoebox<>> i_sb = i_reflection_table["shoebox"];
      dials::af::ref<Shoebox<>> e_sb = e_reflection_table["shoebox"];
      i_sb[0].deallocate();
      e_sb[0].deallocate();
    }
    ...
    for (std::size_t img_num = 0; img_num < (std::size_t)num_images; ++img_num) {
      dxtbx::format::Image<double> img =
        incident_params.incident_data->get_corrected_data(img_num);
      dxtbx::format::Image<bool> mask =
        incident_params.incident_data->get_mask(img_num);
      dials::af::shared<scitbx::af::versa<double, scitbx::af::c_grid<2>>> od(n_panels);
      dials::af::shared<scitbx::af::versa<bool, scitbx::af::c_grid<2>>> om(n_panels);
      for (int i = 0; i < n_panels; ++i) {
        od[i] = img.tile(i).data();
        om[i] = mask.tile(i).data();
      }
      incident_shoebox_processor.next_data_only(
        dials::model::Image<double>(od.const_ref(), om.const_ref()));
    }
    for (std::size_t img_num = 0; img_num < (std::size_t)num_images; ++img_num) {
      dxtbx::format::Image<double> img =
        incident_params.empty_data->get_corrected_data(img_num);
  ...
  template <typename ProfileParams>
  boost::python::tuple _calculate_line_profile_3d_with_incident(
    dials::af::reflection_table& reflection,
    Experiment& experiment,
    ImageSequence& data,
    const dials_scaling::TOFIncidentSpectrumParams& incident_params,
    ...
    dials::af::reflection_table i_refl_table, e_refl_table;
    _fill_incident_empty_shoeboxes(
      reflection, incident_params, n_panels, num_images, i_refl_table, e_refl_table);

    int bg_code = Valid | Background | BackgroundUsed;
    dials::af::shared<Shoebox<>> shoeboxes = reflection["shoebox"];
    dials::model::Shoebox<> shoebox = shoeboxes[0];
    Shoebox<> i_shoebox =
      static_cast<dials::af::shared<Shoebox<>>>(i_refl_table["shoebox"])[0];
    Shoebox<> e_shoebox =
      static_cast<dials::af::shared<Shoebox<>>>(e_refl_table["shoebox"])[0];
    ...
    scitbx::af::versa<double, scitbx::af::c_grid<3>> profile_3d_out(
      intensity_3d.accessor());
    if (sum_success) {
      profile_success = fit_profile_3d(coords_3d.const_ref(),
                                       intensity_3d,
                                       background_var_3d,
                                       profile_params,
                                       I_prf,
  ...
  boost::python::tuple calculate_line_profile_for_reflection(
    ...
    const bool& apply_lorentz_correction,
    TOFProfile3DGutmannParams& profile_3d_gutmann_params) {
    return _calculate_line_profile_3d_with_incident(reflection,
                                                    experiment,
                                                    data,
                                                    incident_params,
                                                    coords,
  ...
  template <typename ProfileParams>
  boost::python::tuple _calculate_line_profile_3d_with_incident_absorption(
    dials::af::reflection_table& reflection,
    Experiment& experiment,
    ImageSequence& data,
    const dials_scaling::TOFIncidentSpectrumParams& incident_params,
        ...
        for (std::size_t x = 0; x < shoebox.xsize(); ++x) {
          ...
          if (apply_lorentz_correction) {
            ...
            var_I = (1 / (J * J * T * T)) * var_I0
                    + (I0 * I0 / (J * J * J * J * T * T)) * var_J;
            var_B =
              (1 / (J * J * T * T)) * var_B + (B * B / (J * J * J * J * T * T)) * var_J;
          }
  ...
  boost::python::tuple calculate_line_profile_for_reflection(
    ...
    const bool& apply_lorentz_correction,
    TOFProfile3DGutmannParams& profile_3d_gutmann_params) {
    return _calculate_line_profile_3d_with_incident_absorption(
      reflection,
      experiment,
      data,
      incident_params,
  ...
  boost::python::tuple calculate_line_profile_for_reflection(
    ...
    /*
     * Calculates raw_projected_intensity, projected_intensity,
     * projected_background, sum_intensity, sum_variance with
     * spherical absorption correction and optional Lorentz correction
     */
    ...
    for (std::size_t z = 0; z < shoebox.zsize(); ++z) {
      ...
      for (std::size_t y = 0; y < shoebox.ysize(); ++y) {
        ...
        for (std::size_t x = 0; x < shoebox.xsize(); ++x) {
          ...
          double I = I0 / (bin_width_correction * T);
          double B = raw_B / (bin_width_correction * T);
          I_raw /= (bin_width_correction * T);
          double var_I = var_I0 / (bin_width_correction * bin_width_correction * T * T);
          var_B /= (bin_width_correction * bin_width_correction * T * T);
  ...
  template <typename ProfileParams>
  boost::python::tuple _calculate_line_profile_3d_with_absorption(
    dials::af::reflection_table& reflection,
    Experiment& experiment,
    ImageSequence& data,
    const dials_scaling::TOFAbsorptionParams& corrections_data,
  ...
  boost::python::tuple calculate_line_profile_for_reflection(
    ...
    const bool& apply_lorentz_correction,
    TOFProfile3DGutmannParams& profile_3d_gutmann_params) {
    return _calculate_line_profile_3d_with_absorption(reflection,
                                                      experiment,
                                                      data,
                                                      corrections_data,
                                                      coords,
