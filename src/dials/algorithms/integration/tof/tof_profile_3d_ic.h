...
Ikeda-Carpenter (IC) × Bivariate Gaussian (BVG) model, based on
...
namespace dials { namespace algorithms {
  ...
  struct Profile3DICFunctor {
    ...
    Profile3DICFunctor(
      ...
      num_params = opt_conv_params ? 9 : 7;
      last_params =
    ...
    void extract_params(const Eigen::VectorXd& xc,
      ...
      SigY = std::exp(xc[5]);
      SigP = xc[6];
      HatWidth = optimize_convolution_params ? std::exp(xc[7]) : fixed_hat_width;
      KConv = optimize_convolution_params ? std::exp(xc[8]) : fixed_kconv;
    }

    double calc_scale(const scitbx::af::shared<double>& ic_conv,
                      const scitbx::af::shared<double>& bvg_vals) const {
    ...
    int operator()(const Eigen::VectorXd& x, Eigen::VectorXd& fvec) const {
      ...
      if (!cache_valid || (xc - last_params).cwiseAbs().maxCoeff() > 1e-6) {
        cached_Scale = calc_scale(ic_conv, bvg_vals);
        last_params = xc;
        cache_valid = true;
      }
  ...
  public:
    ...
    TOFProfile3DIC(scitbx::af::const_ref<vec3<double>, af::c_grid<3>> coords_,
      ...
      // Bounds array: (logA, logB, R, T0, logSigX, logSigY, SigP, logHatW, logKConv)
    ...
    scitbx::af::versa<double, af::c_grid<3>> result() const {
      ...
      if (functor->optimize_convolution_params && params.size() >= 9) {
        HatWidth = std::exp(params[7]);
        KConv = std::exp(params[8]);
      }
      const scitbx::af::shared<double> ic_conv =
    ...
    bool trust_result(double error, double I_prf, bool show_error = false) const {
      ...
      if (data_peak > 0.0 && std::abs(profile_peak - data_peak) > 0.5 * data_peak) {
        if (show_error)
