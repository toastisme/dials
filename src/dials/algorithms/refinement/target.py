...
class TargetFactory:
    ...
    def from_parameters_and_experiments(
        ...
        # Determine whether the target is in X, Y, Phi space, X,Y, wavelength,
        #  or just X, Y to choose the right Target to instantiate
        if params.laue:
            targ = LaueLeastSquaresResidualWithRmsdCutoff
        elif isinstance(reflection_manager, StillsReflectionManager):
            if do_sparse:
